...
function NodeStorageCard({ node }: { node: Node }) {
  ...
  return (
    <div className="rounded-lg bg-slate-800 p-4">
      ...
      <div className="mt-2">
        <h4 className="text-xs font-medium uppercase text-slate-400">Commit Log ({commitLog.length})</h4>
      ...
      <div className="mt-3">
        <h4 className="text-xs font-medium uppercase text-slate-400">Memtable ({memtable.length})</h4>
        {memtable.length === 0 ? (
        ...
        {sstables.length === 0 ? (
          ...
          <ul className="mt-1 space-y-2">
            {sstables.map((sstable) => (
              <li key={sstable.id} className="rounded bg-slate-700 p-2 text-xs">
...
export function StorageView({ cluster }: StorageViewProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {selectedNode ? (
        <NodeStorageCard node={selectedNode} />
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {cluster.nodes.map((node) => (
            <NodeStorageCard key={node.id} node={node} />
          ))}
        </div>
      )}
    </div>
