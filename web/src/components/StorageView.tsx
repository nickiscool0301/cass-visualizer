...
function NodeStorageCard({
  ...
  return (
    <div className={`border-b pb-4 transition-all duration-300 ${isWriteTarget || isFlushed ? "animate-glow" : ""}`} style={{ borderColor: "var(--border-subtle)" }}>
      ...
      <div className="mt-3">
        ...
        <ul className="mt-1 max-h-20 space-y-0.5 overflow-y-auto rounded border p-1.5 text-[11px]" style={{ borderColor: "var(--border-subtle)" }}>
          {commitLog.slice(0, 5).map((row, i) => (
            <li key={i} className={`${i === 0 && isWriteTarget ? "animate-slide-in" : ""}`} style={{ color: "var(--text-secondary)" }}>
              <span style={{ color: "var(--accent)" }}>{row.partitionKey}</span>{" "}
      ...
      <div className="mt-3">
        ...
        {memtable.length === 0 ? (
          <p className="mt-1 rounded border p-1.5 text-[11px]" style={{ color: "var(--text-tertiary)", borderColor: "var(--border-subtle)" }}>
            Empty
          </p>
        ) : (
          <table className="mt-1 w-full text-[11px]">
            ...
            <tbody>
              {memtable.map((row, i) => (
                ...
                >
                  <td className="py-1" style={{ color: "var(--accent)" }}>
                    {row.partitionKey}
                  </td>
            ...
            {sstables.map((sstable, i) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-mono-data" style={{ color: "var(--text-tertiary)" }}>
                    {sstable.id}
                  </span>
                  <span style={{ color: "var(--text-secondary)" }}>{sstable.rows.length} rows</span>
                </div>
              </li>
            ))}
          </ul>
